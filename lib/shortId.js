...
module.exports = function(params) {
	...
	// Setters
	const setBase = function(set) {
		baseCharSet = new Utils.BaseCharSet(set);
	}

	const setEncoding = function(set) {
		encodingCharSet = new Utils.EncodingCharSet(set);
	}


	const setFullId = function(value) {
		isFullId = !!value;
		if (isFullId)
		{
	...
	const BitStream = function(input, base) {
		var bitBuffer = [];
		var position = 0;
	...
	return {
		...
		decode: function(input) {
			...
			while (!bitStream.isEOF())
			{
				...
				const char = baseCharSet.set.charAt(value);
				result += char;
			}
